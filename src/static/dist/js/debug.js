...
Elm.Slider.make = function (_elm) {
   ...
   var defaultSlider = {horizontal: true
                       ,disabled: false
                       ,length: 100
                       ,min: 0
                       ,max: 100
                       ,step: 1
                       ,value: 0};
   var SliderStyle = F7(function (a,b,c,d,e,f,g) {
      return {horizontal: a
             ,disabled: b
             ,length: c
             ,min: d
...
Elm.SideBar.Model = Elm.SideBar.Model || {};
Elm.SideBar.Model.make = function (_elm) {
   "use strict";
   _elm.SideBar = _elm.SideBar || {};
   _elm.SideBar.Model = _elm.SideBar.Model || {};
   if (_elm.SideBar.Model.values) return _elm.SideBar.Model.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $List = Elm.List.make(_elm),
   ...
   var update = F2(function (action,state) {
      var _p0 = action;
      switch (_p0.ctor)
      {case "Restart": return _U.update(state,
           {totalEvents: 0,scrubPosition: 0});
         case "Pause": var _p1 = _p0._0;
           return _U.update(state,
           {paused: _p1
           ,totalEvents: _p1 ? state.totalEvents : state.scrubPosition});
         case "TotalEvents": var _p2 = _p0._0;
           return _U.update(state,{totalEvents: _p2,scrubPosition: _p2});
         default: return _U.update(state,
           {scrubPosition: _p0._0,paused: true});}
   });
   var startModel = {paused: false
                    ,totalEvents: 0
                    ,scrubPosition: 0};
   var Model = F3(function (a,b,c) {
      return {paused: a,totalEvents: b,scrubPosition: c};
   });
   var ScrubPosition = function (a) {
      return {ctor: "ScrubPosition",_0: a};
   };
   var TotalEvents = function (a) {
      return {ctor: "TotalEvents",_0: a};
   };
   var Pause = function (a) {    return {ctor: "Pause",_0: a};};
   var Restart = {ctor: "Restart"};
   return _elm.SideBar.Model.values = {_op: _op
                                      ,Restart: Restart
                                      ,Pause: Pause
...
Elm.SideBar = Elm.SideBar || {};
Elm.SideBar.Controls = Elm.SideBar.Controls || {};
Elm.SideBar.Controls.make = function (_elm) {
   "use strict";
   _elm.SideBar = _elm.SideBar || {};
   _elm.SideBar.Controls = _elm.SideBar.Controls || {};
   if (_elm.SideBar.Controls.values)
   return _elm.SideBar.Controls.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Color = Elm.Color.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   ...
   $Text = Elm.Text.make(_elm);
   var _op = {};
   var roundedSquare = F3(function (side,radius,toForm) {
      var formedCircle = toForm($Graphics$Collage.circle(radius));
      var shortSide = side - 2 * radius;
      var xRect = toForm(A2($Graphics$Collage.rect,side,shortSide));
      var yRect = toForm(A2($Graphics$Collage.rect,shortSide,side));
      var circleOffset = shortSide / 2;
      var tl = A2($Graphics$Collage.move,
      {ctor: "_Tuple2",_0: 0 - circleOffset,_1: circleOffset},
      formedCircle);
      var tr = A2($Graphics$Collage.move,
      {ctor: "_Tuple2",_0: circleOffset,_1: circleOffset},
      formedCircle);
      var bl = A2($Graphics$Collage.move,
      {ctor: "_Tuple2",_0: 0 - circleOffset,_1: 0 - circleOffset},
      formedCircle);
      var br = A2($Graphics$Collage.move,
      {ctor: "_Tuple2",_0: circleOffset,_1: 0 - circleOffset},
      formedCircle);
      return $Graphics$Collage.group(_U.list([xRect
                                             ,yRect
                                             ,tl
                                             ,tr
                                             ,bl
   ...
   var scrubMailbox = $Signal.mailbox(0);
   var scrubSlider = F2(function (_p0,state) {
      var _p1 = _p0;
      var sliderLength = _p1._0;
      var sliderStyle = _U.update($Slider.defaultSlider,
      {length: sliderLength
      ,max: $Basics.toFloat(state.totalEvents)
      ,value: $Basics.toFloat(state.scrubPosition)});
      ...
      function (n) {
         ...
         scrubMailbox.address,
         $Basics.round(n));
      },
      sliderStyle));
   });
   var permitSwapMailbox = $Signal.mailbox(true);
   var restartMailbox = $Signal.mailbox({ctor: "_Tuple0"});
   var pausedInputMailbox = $Signal.mailbox(false);
   var myButton = F2(function (message,name) {
      var img = function (state) {
         return A3($Graphics$Element.image,
         40,
         40,
         A2($Basics._op["++"],
         "http://localhost:8000/_reactor/debugger/",
         A2($Basics._op["++"],
         name,
         A2($Basics._op["++"],
         "-button-",
         A2($Basics._op["++"],state,".png")))));
      };
   ...
   A2($Signal.message,pausedInputMailbox.address,false),
   "play");
   var pauseButton = A2(myButton,
   A2($Signal.message,pausedInputMailbox.address,true),
   "pause");
   var restartButton = A2(myButton,
   A2($Signal.message,restartMailbox.address,{ctor: "_Tuple0"}),
   "restart");
   var textTypefaces = _U.list(["Gotham"
                               ,"Futura"
                               ,"Lucida Grande"
                               ,"sans-serif"]);
   var darkGrey = A3($Color.rgb,74,74,74);
   var lightGrey = A3($Color.rgb,228,228,228);
   var dataStyle = F3(function (typefaces,height,string) {
      var $default = $Text.defaultStyle;
      var myStyle = _U.update($default,
      {typeface: typefaces
      ,color: lightGrey
      ,height: $Maybe.Just(height)});
      return A2($Text.style,myStyle,$Text.fromString(string));
   });
   var textStyle = function (string) {
      return A3(dataStyle,textTypefaces,12,string);
   };
   var blue = A3($Color.rgb,28,129,218);
   var swapButton = function (permitSwap) {
      var info = $Graphics$Element.leftAligned(textStyle("swap"));
      var radius = 4;
      var hsWidth = 25;
      ...
      var trueButton = _U.list([bgButton
                               ,A3(roundedSquare,22,radius,$Graphics$Collage.filled(blue))]);
      var falseButtonClick = trueButton;
      var trueButtonHover = _U.list([bgButton
                                    ,A3(roundedSquare,22,radius,$Graphics$Collage.filled(blue))
      ...
      var falseButton = _U.list([bgButton
                                ,A3(roundedSquare,
                                22,
                                radius,
                                $Graphics$Collage.filled(darkGrey))]);
      var trueButtonClick = falseButton;
      var falseButtonHover = _U.list([bgButton
                                     ,A3(roundedSquare,22,radius,$Graphics$Collage.filled(darkGrey))
                                     ,A2($Graphics$Collage.alpha,
                                     0.1,
                                     A3(roundedSquare,22,radius,$Graphics$Collage.filled(blue)))]);
      var button = function () {
         ...
         if (_p2 === true) {
               return A4($Graphics$Input.customButton,
               A2($Signal.message,permitSwapMailbox.address,false),
               A3($Graphics$Collage.collage,hsWidth,hsWidth,trueButton),
               A3($Graphics$Collage.collage,hsWidth,hsWidth,trueButtonHover),
               A3($Graphics$Collage.collage,hsWidth,hsWidth,trueButtonClick));
            } else {
               return A4($Graphics$Input.customButton,
               A2($Signal.message,permitSwapMailbox.address,true),
               A3($Graphics$Collage.collage,hsWidth,hsWidth,falseButton),
               A3($Graphics$Collage.collage,hsWidth,hsWidth,falseButtonHover),
               A3($Graphics$Collage.collage,hsWidth,hsWidth,falseButtonClick));
            }
      }();
      return A2($Graphics$Element.flow,
      $Graphics$Element.right,
      _U.list([info,A2($Graphics$Element.spacer,10,1),button]));
   };
   var panelWidth = 275;
   var textHeight = 20;
   var sliderMinMaxText = F2(function (w,state) {
      var sliderTotalEvents = A4($Graphics$Element.container,
      w,
      textHeight,
      $Graphics$Element.topRight,
      $Graphics$Element.rightAligned(textStyle($Basics.toString(state.totalEvents))));
      var sliderStartText = A4($Graphics$Element.container,
      w,
      textHeight,
      $Graphics$Element.topLeft,
      $Graphics$Element.leftAligned(textStyle("0")));
      return A2($Graphics$Element.flow,
      $Graphics$Element.outward,
      _U.list([sliderStartText,sliderTotalEvents]));
   });
   var sideMargin = 2 * 20;
   var sliderEventText = F2(function (w,state) {
      var text$ = $Graphics$Element.centered(textStyle($Basics.toString(state.scrubPosition)));
      ...
      var leftDistance = function () {
         var _p3 = totalEvents;
         if (_p3 === 0) {
               return sideMargin / 2 + textWidthOffset / 2;
            } else {
               return scrubPosition / totalEvents * midWidth + sideMargin / 2 + textWidthOffset / 2;
            }
      }();
      var xPos = $Graphics$Element.absolute($Basics.round(leftDistance));
      var textPosition = A2($Graphics$Element.middleAt,xPos,yPos);
      return A4($Graphics$Element.container,
      w,
      textHeight,
      textPosition,
   ...
   var buttonWidth = 40;
   var buttonHeight = 40;
   var view = F4(function (_p4,showSwap,permitSwap,state) {
      var _p5 = _p4;
      var _p6 = _p5._0;
      var bar = A2($Graphics$Element.opacity,
      0.3,
      A2($Graphics$Element.color,
      lightGrey,
      A2($Graphics$Element.spacer,_p6,1)));
      var fittedSwapButton = showSwap ? A4($Graphics$Element.container,
      _p6 - 2 * buttonWidth - sideMargin,
      buttonHeight,
      $Graphics$Element.middle,
      swapButton(permitSwap)) : A2($Graphics$Element.spacer,
      2 * buttonWidth,
      1);
      var buttons = A2($Graphics$Element.flow,
      $Graphics$Element.right,
      _U.list([restartButton
              ,fittedSwapButton
      ...
      var midWidth = _p6 - sideMargin;
      var centeredSliderContainer = A4($Graphics$Element.container,
      _p6,
      24 + textHeight,
      $Graphics$Element.midTop,
      A2($Graphics$Element.flow,
      $Graphics$Element.down,
      _U.list([A2(scrubSlider,
              {ctor: "_Tuple2",_0: midWidth,_1: _p5._1},
              state)
              ,A2(sliderMinMaxText,midWidth,state)])));
      var slider = A4($Graphics$Element.container,
      _p6,
      24 + 2 * textHeight,
      $Graphics$Element.midTop,
      A2($Graphics$Element.flow,
      $Graphics$Element.down,
      _U.list([A2(sliderEventText,_p6,state)
              ,centeredSliderContainer])));
      var controls = A2($Graphics$Element.flow,
      $Graphics$Element.down,
      _U.list([A2($Graphics$Element.spacer,midWidth,topSpacerHeight)
              ,buttonContainer
              ,A2($Graphics$Element.spacer,midWidth,buttonSliderSpaceHeight)
              ,slider
              ,A2($Graphics$Element.spacer,midWidth,10)]));
      return A2($Graphics$Element.flow,
      $Graphics$Element.down,
      _U.list([controls,bar]));
   });
   return _elm.SideBar.Controls.values = {_op: _op
                                         ...
                                         ,darkGrey: darkGrey
                                         ,dataStyle: dataStyle
                                         ,textTypefaces: textTypefaces
                                         ,textStyle: textStyle
                                         ,myButton: myButton
                                         ,playButton: playButton
                                         ,pauseButton: pauseButton
                                         ,pausedInputMailbox: pausedInputMailbox
                                         ,restartButton: restartButton
                                         ,restartMailbox: restartMailbox
                                         ,swapButton: swapButton
                                         ,permitSwapMailbox: permitSwapMailbox
                                         ,scrubSlider: scrubSlider
                                         ,scrubMailbox: scrubMailbox
...
Elm.SideBar = Elm.SideBar || {};
Elm.SideBar.Watches = Elm.SideBar.Watches || {};
Elm.SideBar.Watches.make = function (_elm) {
   "use strict";
   _elm.SideBar = _elm.SideBar || {};
   _elm.SideBar.Watches = _elm.SideBar.Watches || {};
   if (_elm.SideBar.Watches.values)
   return _elm.SideBar.Watches.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Html = Elm.Html.make(_elm),
   ...
   var valueAttributes = _U.list([$Html$Attributes.style(_U.list([{ctor: "_Tuple2"
                                                                  ,_0: "margin"
                                                                  ,_1: "0 0 0 10px"}]))]);
   var viewValue = function (value) {
      return A2($Html.pre,
      valueAttributes,
      _U.list([$Html.text(value)]));
   };
   var nameAttributes = _U.list([$Html$Attributes.style(_U.list([{ctor: "_Tuple2"
                                                                 ,_0: "margin"
                                                                 ,_1: "20px 0 10px"}
                                                                ,{ctor: "_Tuple2",_0: "font-weight",_1: "bold"}
                                                                ,{ctor: "_Tuple2"
                                                                 ,_0: "font-family"
                                                                 ,_1: "Gotham, Futura, \'Lucida Grande\', sans-serif"}]))]);
   var viewName = function (name) {
      return A2($Html.div,
      nameAttributes,
      _U.list([$Html.text(name)]));
   };
   var watchAttributes = _U.list([$Html$Attributes.style(_U.list([{ctor: "_Tuple2"
                                                                  ,_0: "color"
                                                                  ,_1: "rgb(228, 228, 228)"}]))]);
   var viewWatch = function (_p0) {
      var _p1 = _p0;
      return A2($Html.div,
      watchAttributes,
      _U.list([viewName(_p1._0),viewValue(_p1._1)]));
   };
   var viewAttributes = _U.list([$Html$Attributes.style(_U.list([{ctor: "_Tuple2"
                                                                 ,_0: "overflow-y"
                                                                 ,_1: "auto"}
                                                                ,{ctor: "_Tuple2",_0: "overflow-x",_1: "hidden"}
                                                                ,{ctor: "_Tuple2",_0: "height",_1: "100%"}
                                                                ,{ctor: "_Tuple2",_0: "padding",_1: "0 20px"}]))]);
   var view = function (watches) {
      return A2($Html.div,
      viewAttributes,
      function () {
         var _p2 = watches;
         if (_p2.ctor === "[]") {
               return _U.list([noWatches]);
            } else {
               return A2($List.map,viewWatch,watches);
            }
      }());
   };
   return _elm.SideBar.Watches.values = {_op: _op
                                        ,view: view
...
Elm.SideBar.make = function (_elm) {
   "use strict";
   _elm.SideBar = _elm.SideBar || {};
   if (_elm.SideBar.values) return _elm.SideBar.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Graphics$Element = Elm.Graphics.Element.make(_elm),
   ...
   var _op = {};
   var showSwap = Elm.Native.Port.make(_elm).inbound("showSwap",
   "Bool",
   function (v) {
      return typeof v === "boolean" ? v : _U.badPort("a boolean (true or false)",
      v);
   });
   var watches = Elm.Native.Port.make(_elm).inboundSignal("watches",
   "List ( String, String )",
   function (v) {
   ...
   var eventCounter = Elm.Native.Port.make(_elm).inboundSignal("eventCounter",
   "Int",
   function (v) {
      return typeof v === "number" && isFinite(v) && Math.floor(v) === v ? v : _U.badPort("an integer",
      v);
   });
   var scrubMailbox = $SideBar$Controls.scrubMailbox;
   var scrubTo = Elm.Native.Port.make(_elm).outboundSignal("scrubTo",
   function (v) {
      return v;
   },
   scrubMailbox.signal);
   var pausedInputMailbox = $SideBar$Controls.pausedInputMailbox;
   var pause = Elm.Native.Port.make(_elm).outboundSignal("pause",
   function (v) {
      return v;
   },
   pausedInputMailbox.signal);
   var restartMailbox = $SideBar$Controls.restartMailbox;
   var restart = Elm.Native.Port.make(_elm).outboundSignal("restart",
   function (v) {
      return v;
   },
   A2($Signal.map,$Basics.always(0),restartMailbox.signal));
   var permitSwapMailbox = $SideBar$Controls.permitSwapMailbox;
   var permitSwap = Elm.Native.Port.make(_elm).outboundSignal("permitSwap",
   function (v) {
      return v;
   },
   permitSwapMailbox.signal);
   var aggregateUpdates = $Signal.mergeMany(_U.list([A2($Signal.map,
                                                    $Basics.always($SideBar$Model.Restart),
                                                    restartMailbox.signal)
                                                    ,A2($Signal.map,$SideBar$Model.Pause,pausedInputMailbox.signal)
                                                    ,A2($Signal.map,$SideBar$Model.TotalEvents,eventCounter)
                                                    ,A2($Signal.map,
                                                    $SideBar$Model.ScrubPosition,
                                                    scrubMailbox.signal)]));
   var scene = A3($Signal.foldp,
   $SideBar$Model.update,
   $SideBar$Model.startModel,
   aggregateUpdates);
   var view = F4(function (_p0,watches,permitSwap,state) {
      var _p1 = _p0;
      var _p3 = _p1._0;
      var _p2 = _p1._1;
      var watchView = A3($Html.toElement,
      _p3,
      _p2 - 150,
      $SideBar$Watches.view(watches));
      var controls = A4($SideBar$Controls.view,
   ...
   function (_p4) {
      var _p5 = _p4;
      return {ctor: "_Tuple2"
             ,_0: $SideBar$Controls.panelWidth
             ,_1: _p5._1};
   },
   $Window.dimensions),
   watches,
   permitSwapMailbox.signal,
   scene);
   return _elm.SideBar.values = {_op: _op
                                ,view: view
                                ,main: main
                                ,scene: scene
...
Elm.fullscreenDebug = function(moduleName, fileName) {
	...
	// handle swaps
	var updates = 'ws://localhost:8000/socket?file=' + fileName
